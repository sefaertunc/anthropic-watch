...
No server, no database — just static feeds anyone can subscribe to. The current source count is tracked in `docs/SOURCES.md` and the README badge. The repo monitors 37 sources across three categories (Core, Extended, Community) and ten scraper types.
...
- [x] v1.5.1 (2026-05-05) — Reddit OAuth path removed after RBP denied access twice. Scraper rewritten to consume public Atom RSS (`/r/<sub>/<mode>.rss`), bypassing both the OAuth gate and the datacenter-IP block on `*.json` simultaneously. `REDDIT_CLIENT_ID`/`REDDIT_CLIENT_SECRET` and `docs/reddit-oauth-setup.md` removed. r/Claudeopus migrated to `top/week` since Atom exposes no per-post score. Restores compliance with Project Rule 2 ("no OAuth flows beyond `GITHUB_TOKEN`").
