...
**Phase:** Phase 7 — Feed-health observability (v1.5.0 shipped 2026-04-30; institutionalizes the v1.4.2 lesson by publishing a `feed-health.json` artifact and dashboard panel; no active feature work)
...
- [x] Phase 7 — Feed-health observability (v1.5.0, 2026-04-30, PR #24) — Institutionalizes the v1.4.2 lesson with a published `feed-health.json` artifact and dashboard panel exposing pipeline-output integrity distinct from per-source health. Four indicators (`runHistoryDepth`, `allJsonItemCount`, `perSourceFeedContinuity`, `cronFreshness`), three severity states (`ok` / `warning` / `fired`). Per-source uses item-membership (not count-only) so cap-saturated 50-item feeds are correctly checked. `cronFreshness` publishes inputs only — state computed at read time because a stale envelope cannot self-report staleness. Orchestrator wiring runs LAST, after `saveState` and the GHA `has_new_items` output, with a try/catch carve-out so observability code cannot break feed publishing (verified by `test/e2e/feed-health-isolation.test.js`). Server/client `aggregateOverall` parity locked by shared fixture `test/fixtures/aggregate-cases.json` and a fail-CI-on-drift parity test. `readJsonSafe` extracted from duplicated definitions in `src/index.js` and `src/feed/health.js` into `src/read-json-safe.js`. Schema-version policy: additive minor for new indicators, optional fields, and state values; `summary.byState` is an open map. Scope ceiling held: no Slack, email, webhooks, GitHub-Issue auto-opening — v1.5 publishes the structured signal and the documented merge pattern, consumers compose the alerting layer. Docs updated in same PR per Rule 10 (FEED-SCHEMA, ARCHITECTURE, TROUBLESHOOTING, WORCLAUDE-INTEGRATION). 30 new unit tests + 2 new e2e (189/189 passing). Client library wrapper deferred until the schema settles in production.
...
4. Watch `feed-health.json` after first post-v1.5.0 cron — `runHistoryDepth` will read `warning` until `run-history.json` grows back to 90 entries (~12 weeks). Expected; documented in CHANGELOG and TROUBLESHOOTING. Confirm dashboard's Feed Health panel renders four indicators with `cronFreshness` state computed client-side from `generatedAt`.
...
8. Client library `fetchFeedHealth()` deferred — wait for the `feed-health.json` schema to settle in production for ≥2 weeks of real cron runs before wrapping in `@sefaertunc/anthropic-watch-client`. Then ship as `@sefaertunc/anthropic-watch-client@1.0.4` (or wherever the version lands).
